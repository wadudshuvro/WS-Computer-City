...
import {
  buildRamCategoryWhere,
  buildRamFilterCounts,
  buildRamSpecCondition,
  RAM_SPEC_FILTER_KEYS,
} from '@/lib/ramFilterQuery';

/**
 * GET /api/products/ram
 * Desktop RAM products with Star Tech-style sidebar filtering
 */
export async function GET(req: NextRequest) {
  try {
    ...
    RAM_SPEC_FILTER_KEYS.forEach((key) => {
    ...
    const categoryWhere = await buildRamCategoryWhere(sub, type);
    ...
    if (Object.keys(specFilters).length > 0) {
      ...
      for (const [key, values] of Object.entries(specFilters)) {
        const condition = buildRamSpecCondition(key, values);
        if (condition) {
    ...
    const [products, total, priceAggregation, filterCounts] = await Promise.all([
      ...
      prisma.product.aggregate({
        where: { isActive: true, ...(await buildRamCategoryWhere(sub, type)) },
        _min: { price: true },
        _max: { price: true },
      }),
...
async function getFilterCounts(
  ...
  try {
    const baseWhere: Prisma.ProductWhereInput = {
      isActive: true,
      ...(await buildRamCategoryWhere(sub, type)),
    };
    ...
    for (const key of RAM_SPEC_FILTER_KEYS) {
    ...
    return buildRamFilterCounts(specValuesByKey, stockCounts, brandCounts);
  } catch (error) {
    console.error('Error getting RAM filter counts:', error);
    return {};
  }
}
