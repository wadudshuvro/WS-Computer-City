...
import {
  buildSsdCategoryWhere,
  buildSsdFilterCounts,
  buildSsdSpecCondition,
  SSD_SPEC_FILTER_KEYS,
} from '@/lib/ssdFilterQuery';

/**
 * GET /api/products/ssd
 * SSD products with Star Tech-style sidebar filtering
 */
export async function GET(req: NextRequest) {
  try {
    ...
    SSD_SPEC_FILTER_KEYS.forEach((key) => {
    ...
    const categoryWhere = await buildSsdCategoryWhere(sub, type);
    ...
    if (Object.keys(specFilters).length > 0) {
      ...
      for (const [key, values] of Object.entries(specFilters)) {
        const condition = buildSsdSpecCondition(key, values);
        if (condition) {
    ...
    const [products, total, priceAggregation, filterCounts] = await Promise.all([
      ...
      prisma.product.aggregate({
        where: { isActive: true, ...(await buildSsdCategoryWhere(sub, type)) },
        _min: { price: true },
        _max: { price: true },
      }),
...
async function getFilterCounts(
  ...
  try {
    const baseWhere: Prisma.ProductWhereInput = {
      isActive: true,
      ...(await buildSsdCategoryWhere(sub, type)),
    };
    ...
    for (const key of SSD_SPEC_FILTER_KEYS) {
    ...
    return buildSsdFilterCounts(specValuesByKey, stockCounts, brandCounts);
  } catch (error) {
    console.error('Error getting SSD filter counts:', error);
    return {};
  }
}
