...
import {
  buildCasingCategoryWhere,
  buildCasingFilterCounts,
  buildCasingSpecCondition,
  CASING_SPEC_FILTER_KEYS,
} from '@/lib/casingFilterQuery';

/**
 * GET /api/products/casing
 * Computer Casing products with Star Tech-style sidebar filtering
 */
export async function GET(req: NextRequest) {
  try {
    ...
    CASING_SPEC_FILTER_KEYS.forEach((key) => {
    ...
    const categoryWhere = await buildCasingCategoryWhere(sub, type);
    ...
    if (Object.keys(specFilters).length > 0) {
      ...
      for (const [key, values] of Object.entries(specFilters)) {
        const condition = buildCasingSpecCondition(key, values);
        if (condition) {
    ...
    const [products, total, priceAggregation, filterCounts] = await Promise.all([
      ...
      prisma.product.aggregate({
        where: { isActive: true, ...(await buildCasingCategoryWhere(sub, type)) },
        _min: { price: true },
        _max: { price: true },
      }),
    ...
    return NextResponse.json({
      ...
      filters: {
        priceRange: {
          ...
          max: priceAggregation._max.price ? Number(priceAggregation._max.price) : 300000,
        },
        counts: filterCounts,
      },
    });
  } catch (error) {
    console.error('Error fetching casing products:', error);
    return NextResponse.json(
      {
        error: {
          code: 'INTERNAL_ERROR',
...
async function getFilterCounts(
  ...
  try {
    const baseWhere: Prisma.ProductWhereInput = {
      isActive: true,
      ...(await buildCasingCategoryWhere(sub, type)),
    };
    ...
    for (const key of CASING_SPEC_FILTER_KEYS) {
    ...
    return buildCasingFilterCounts(specValuesByKey, stockCounts, brandCounts);
  } catch (error) {
    console.error('Error getting casing filter counts:', error);
    return {};
  }
}
