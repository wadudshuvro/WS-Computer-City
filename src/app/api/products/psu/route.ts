...
import {
  buildPsuCategoryWhere,
  buildPsuFilterCounts,
  buildPsuSpecCondition,
  PSU_SPEC_FILTER_KEYS,
} from '@/lib/psuFilterQuery';

/**
 * GET /api/products/psu
 * Power Supply products with Star Tech-style sidebar filtering
 */
export async function GET(req: NextRequest) {
  try {
    ...
    PSU_SPEC_FILTER_KEYS.forEach((key) => {
    ...
    const categoryWhere = await buildPsuCategoryWhere(sub, type);
    ...
    if (Object.keys(specFilters).length > 0) {
      ...
      for (const [key, values] of Object.entries(specFilters)) {
        const condition = buildPsuSpecCondition(key, values);
        if (condition) {
    ...
    const [products, total, priceAggregation, filterCounts] = await Promise.all([
      ...
      prisma.product.aggregate({
        where: { isActive: true, ...(await buildPsuCategoryWhere(sub, type)) },
        _min: { price: true },
        _max: { price: true },
      }),
    ...
    return NextResponse.json({
      ...
      filters: {
        priceRange: {
          ...
          max: priceAggregation._max.price ? Number(priceAggregation._max.price) : 500000,
        },
        counts: filterCounts,
      },
    });
  } catch (error) {
    console.error('Error fetching PSU products:', error);
    return NextResponse.json(
      {
        error: {
          code: 'INTERNAL_ERROR',
...
async function getFilterCounts(
  ...
  try {
    const baseWhere: Prisma.ProductWhereInput = {
      isActive: true,
      ...(await buildPsuCategoryWhere(sub, type)),
    };
    ...
    for (const key of PSU_SPEC_FILTER_KEYS) {
    ...
    return buildPsuFilterCounts(specValuesByKey, stockCounts, brandCounts);
  } catch (error) {
    console.error('Error getting PSU filter counts:', error);
    return {};
  }
}
